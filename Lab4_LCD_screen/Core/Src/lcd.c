...
void My_Delay(uint32_t mysec)
{
	...
	TIM5->CR1 &=  ~(1 << 0);  ;//stopp timmer
	TIM5->CNT = 0; // nollställa counter
	TIM5->CR1 |=(1 << 0); //kör timmr
	while (TIM5->CNT < mysec)
	{
		 //__NOP(); // wait
	}
	...
	while (__HAL_TIM_GET_COUNTER(&htim5) < mysec);
	*/
...
void TextLCD_PutStr		(TextLCDType * hlcd, char * str)
{
	while(*str)
	{
		TextLCD_SendByte(hlcd, *str, GPIO_PIN_SET);
		str++;
	}
}
